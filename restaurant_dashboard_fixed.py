...
# Custom styled dataframe without matplotlib dependency
def styled_dataframe(df, height=300):
    ...
    for _, row in df.head(20).iterrows():
        ...
        for val in row:
            ...
            if isinstance(val, (int, float)):
                if val > 1000:  # Assume currency if large number
                    html += f"<td style='font-weight: 600; color: #2c3e50;'>${val:,.2f}</td>"
                else:
                    html += f"<td>{val:,.2f}</td>"
            else:
                html += f"<td>{val}</td>"
    ...
    if len(df) > 20:
        st.caption(f"Showing 20 of {len(df)} rows. Use filters to see more specific data.")
...
def main():
    ...
    with st.sidebar:
        ...
        if len(filtered_df) > 0:
            ...
            top_category_rev = filtered_df.groupby('Category')['Revenue'].sum().max()
            st.info(f"**Top Category:** {top_category} (${top_category_rev:,.0f})")
        
        # Busiest day
        if len(filtered_df) > 0:
            busiest_day = filtered_df['Order Weekday'].mode()[0] if 'Order Weekday' in filtered_df.columns else 'N/A'
    ...
    with tab1:
        ...
        with col2:
            ...
            avg_orders = len(df) / ((df['Order Date'].max() - df['Order Date'].min()).days + 1) * (end_date - start_date).days if len(date_range) == 2 else len(df)
        ...
        if len(filtered_df) > 0:
            # Ensure we have datetime sorted
            filtered_df_sorted = filtered_df.sort_values('Order Date')
            
            # Daily revenue
            daily_revenue = filtered_df_sorted.resample('D', on='Order Date')['Revenue'].sum().reset_index()
            daily_revenue['MA7'] = daily_revenue['Revenue'].rolling(window=7, min_periods=1).mean()
            
            # Weekly revenue
            weekly_revenue = filtered_df_sorted.resample('W', on='Order Date')['Revenue'].sum().reset_index()
            
            # Monthly revenue
            monthly_revenue = filtered_df_sorted.resample('M', on='Order Date')['Revenue'].sum().reset_index()
        ...
        if len(filtered_df) > 0:
            ...
            st.info("Select filters to see item performance data.")
    ...
    with tab2:
        ...
        if len(filtered_df) > 0:
            ...
            with col2:
                ...
                display_df = category_items.sort_values('Revenue', ascending=False).head(10)[
                    ['Item', 'Revenue', 'Orders', 'Quantity', 'Price']
                ].copy()
                display_df['Revenue'] = display_df['Revenue'].apply(lambda x: f"${x:,.2f}")
                display_df['Price'] = display_df['Price'].apply(lambda x: f"${x:,.2f}")
                
                styled_dataframe(display_df, height=400)
        else:
            st.info("Select filters to see category analysis data.")
    ...
    with tab3:
        ...
        if len(filtered_df) > 0:
            ...
            with col2:
                ...
                retention_rate = len(customer_stats[customer_stats['Total Orders'] > 1]) / len(customer_stats) * 100 if len(customer_stats) > 0 else 0
                ...
                # Format the display
                display_customers = top_customers[['Customer ID', 'Total Revenue', 'Total Orders', 'Avg Order Value']].copy()
                display_customers['Total Revenue'] = display_customers['Total Revenue'].apply(lambda x: f"${x:,.2f}")
                display_customers['Avg Order Value'] = display_customers['Avg Order Value'].apply(lambda x: f"${x:,.2f}")
                
                styled_dataframe(display_customers, height=200)
        else:
            st.info("Select filters to see customer insights data.")
    ...
    with tab4:
        ...
        if len(filtered_df) > 0:
            ...
            st.info("Select filters to see temporal analysis data.")
    ...
    with tab5:
        ...
        if len(filtered_df) > 0:
            ...
            if columns:
                ...
                display_data = filtered_df[columns].sort_values('Order Date', ascending=False).head(100).copy()
                
                # Format numeric columns
                for col in ['Price', 'Order Total']:
                ...
                styled_dataframe(display_data, height=400)
            ...
            if numeric_cols:
                ...
                styled_dataframe(formatted_stats, height=300)
            else:
            ...
            with col2:
                if st.button("📊 Export Summary Statistics", use_container_width=True):
                    # Create summary statistics
                    summary_stats = pd.DataFrame({
                        'Metric': ['Total Orders', 'Total Revenue', 'Unique Customers', 'Average Order Value'],
                        'Value': [
                            len(filtered_df),
                    ...
                    st.download_button(
                        ...
                        data=csv,
                        file_name="restaurant_summary_stats.csv",
                        mime="text/csv"
                    )
        else:
    ...
    <div class="footer">
        ...
        <p><span class="badge badge-primary">Academic Grade</span> 
           <span class="badge badge-success">Professional</span> 
           <span class="badge badge-warning">Interactive</span> 
           <span class="badge badge-danger">Production Ready</span></p>
    </div>
    """, unsafe_allow_html=True)
